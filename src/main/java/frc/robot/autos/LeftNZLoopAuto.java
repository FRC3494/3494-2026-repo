...
public class LeftNZLoopAuto {
  public static AutoRoutine getRoutine(
    ...
    AutoTrajectory leftNZLoop =
        alliance == Alliance.Blue
            ? ChoreoTraj.LeftNZLoop_BLUE.asAutoTraj(routine)
            : ChoreoTraj.LeftNZLoop_RED.asAutoTraj(routine);
    AutoTrajectory leftNZLoopToShoot =
        alliance == Alliance.Blue
            ? ChoreoTraj.LeftNZLoopToShoot_BLUE.asAutoTraj(routine)
            : ChoreoTraj.LeftNZLoopToShoot_RED.asAutoTraj(routine);
    ...
    leftTrenchToNZ.done().onTrue(leftNZLoop.cmd());

    leftNZLoop.done().onTrue(sequence(robotCommands.stopIntake(), leftNZLoopToShoot.cmd()));
